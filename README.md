...
`Note! These steps were not tested to work out of the box on MAC`<br>
...
Throughout the exampled helm installations we'll use `jfrog-splunk` as an example namespace. That said, you can use a different or existing namespace instead by setting the following environment variable
...
1. Skip this step if you already have Artifactory installed. Else, install Artifactory using the command below
   ...
   :bulb: Metrics collection is disabled by default in Artifactory. Please make sure that you are following the above `helm upgrade` command to enable them in Artifactory by setting `artifactory.metrics.enabled=true`. For Artifactory versions <=7.86.x, please enable metrics by setting the flag `artifactory.openMetrics.enabled=true`<br>
...
2. Skip this step if you already have Artifactory installed. Else, install Artifactory using the command below
   ...
   :bulb: Metrics collection is disabled by default in Artifactory-HA. Please make sure that you are following the above `helm upgrade` command to enable them in Artifactory by setting `artifactory.metrics.enabled=true`. For Artifactory versions <=7.86.x, please enable metrics by setting the flag `artifactory.openMetrics.enabled=true`<br>
...
Use the same `joinKey` as you used in Artifactory installation ($JOIN_KEY) to allow Xray node to successfully connect to Artifactory.
